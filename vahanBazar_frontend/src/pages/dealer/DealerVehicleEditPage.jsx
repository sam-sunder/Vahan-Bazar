...
const Breadcrumb = () => (
    <nav className="mb-8 text-sm font-medium text-gray-500" aria-label="Breadcrumb">
        <ol className="list-none p-0 inline-flex">
            <li className="flex items-center"><Link to="/dealer/dashboard" className="hover:text-blue-600">Dashboard</Link><ChevronRight size={16} className="mx-2" /></li>
...
const EditVehiclePage = () => {
  ...
  const validateTab = (tabName) => {
    ...
    //   if (!formData.variant) newErrors.variant = 'Variant is required';
      ...
      if (!formData.type) newErrors.category = 'Category is required';
    ...
    const loadVehicleData = async () => {
      try {
        ...
        setFormData({
          ...
          brand: vehicleData.brand_detail.id,
          model: vehicleData.model?.id || 'add-new',
          variant: vehicleData.variant?.id || '',
          price: vehicleData.price.toString(),
          fuel_type: vehicleData.fuel_type || '',
          status: vehicleData.status,
          branch: vehicleData.branch?.id || '',
          featured: vehicleData.is_featured,
          discountType: vehicleData.discount_type || '',
          discountValue: vehicleData.discount_value?.toString() || '',
          discountDescription: vehicleData.discount_description || '',
          type: vehicleData.type,
          year: vehicleData.year?.toString() || '',
          km_driven: vehicleData.km_driven?.toString() || '',
          condition: vehicleData.condition || '',
          exchange_offer: vehicleData.exchange_offer || false,
          loan_option: vehicleData.loan_option || false,
          approved: vehicleData.approved || false,
        });

          setNewModelName(vehicleData.model_name);
        // Set categorization
        setCategorization({
          type: vehicleData.category,
          features: vehicleData.features || defaultFeatures[vehicleData.category] || []
        });

        // Set specifications
        setCustomSpecs(vehicleData.specs || {});

        // Set images
        setImages(vehicleData.images.map(img => ({
          url: img.image,
        ...
        setErrors(prev => ({ ...prev, load: 'Failed to load vehicle data' }));
      }
    };
...
//   }, [formData.variant, formData.model, categorization.type, baseSpecs, variantSpecs]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(images)
      // Convert existing images (with existingImage: true) to File objects if needed
      for (const image of images) {
        if (image.file) {
          // New image uploaded by user
          finalFormData.append('images', image.file);
        } else if (image.existingImage && image.url) {
          // Existing image from DB, fetch and convert to File
          try {
        const response = await fetch(image.url);
        const blob = await response.blob();
        // Use original name if available, otherwise fallback
        const fileName = image.name || image.url.split('/').pop() || 'image.jpg';
        const file = new File([blob], fileName, { type: blob.type });
        finalFormData.append('images', file);
          } catch (err) {
        console.error('Failed to fetch existing image:', image.url, err);
          }
        }
      }
      if (id) {
        await axios.patch(`${API_BASE_URL}/vehicles/${id}/`, finalFormData, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
            'Content-Type': 'multipart/form-data'
          }
    ...
    <div className="space-y-8 mb-8">
      {loading && (
        ...
        <div className="flex justify-between items-start">
            <h1 className="text-4xl font-bold text-gray-800">{id ? 'Edit Vehicle' : 'Add New Vehicle'}</h1>
